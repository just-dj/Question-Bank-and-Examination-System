...
public class DataSourceAspect {
	...
	public void setTxAdvice(TransactionInterceptor txAdvice) throws Exception {
		logger.info("为主从数据库管理切面注入管理策略。");
		if (txAdvice == null) {
			logger.error("注入事务管理策略失败！！管理策略为null");
			System.err.println();
			// 没有配置事务管理策略
			return;
		}
	...
	public void before(JoinPoint point) {
		...
		logger.info("织入切面并执行！编织入方法名为：" + methodName);
		...
		if (isSlave) {
			...
			logger.info("方法 " +  methodName+  " 数据库操作走从库！");
		} else {
			// 标记为写库
			DynamicDataSourceHolder.markMaster();
			logger.info("方法 " +  methodName+  " 数据库操作走主库！");
		}
	}
